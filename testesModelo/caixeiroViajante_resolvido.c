...
int custo (int *v, int n) {
    ...
    for (int i = 0; i < n - 1; i++) {
        custoTotal += (*(v + i) - *(v + i + 1)) * (*(v + i) - *(v + i + 1));
    }

    custoTotal += (*(v + n - 1) - *v) * (*(v + n - 1) - *v);
...
int main()
{
    ...
    const int TAMANHO_DO_VETOR = 9;
    const int NUMERO_DE_ITERACOES = 500;

    int *solucaoNatural = vetorNatural(TAMANHO_DO_VETOR);
    int custoSolucaoNatural = custo(solucaoNatural, TAMANHO_DO_VETOR);

    printf("Solução natural é: ");
    mostrarvetor(solucaoNatural, TAMANHO_DO_VETOR);
    printf("O custo da solução natural é: %d unidades.\n", custoSolucaoNatural);
    ...
    int *melhorSolucao = vetorNatural(TAMANHO_DO_VETOR); 
    int custoMelhorSolucao = custoSolucaoNatural;

    for (int i = 0; i < NUMERO_DE_ITERACOES; i++) {
        int *solucao = vetorNatural(TAMANHO_DO_VETOR);
        baralharvetor(solucao, TAMANHO_DO_VETOR);
        int custoSolucao = custo(solucao, TAMANHO_DO_VETOR);

        if (custoSolucao < custoMelhorSolucao) {
            copiarvetor(solucao, melhorSolucao, TAMANHO_DO_VETOR);
            custoMelhorSolucao = custoSolucao;
        }
    }

    printf("A melhor solução encontrada é: ");
    mostrarvetor(melhorSolucao, TAMANHO_DO_VETOR);
    printf("O custo da melhor solução encontrada é: %d unidades.\n", custoMelhorSolucao);
